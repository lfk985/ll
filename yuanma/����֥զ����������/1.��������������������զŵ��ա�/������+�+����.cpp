...
/***********叶子结点的编码***********/
void CrtHuffmanNodeCode(Huffman ht, char ch[], HuffmanCode h,
                        WeightNode weight, int m, int n) {
  int i, c, p, start;
  char *cd;
...
/*********所有字符的编码*********/
void CrtHuffmanCode(char ch[], HuffmanCode h, HuffmanCode hc,
                    WeightNode weight, int n, int m) {
  int i, k;
  for (i = 0; i < m; i++) {
...
int main() {
  ...
  HuffmanCode h, hc;            /* h存放叶子结点的编码，hc
                                   存放所有结点的编码 */
